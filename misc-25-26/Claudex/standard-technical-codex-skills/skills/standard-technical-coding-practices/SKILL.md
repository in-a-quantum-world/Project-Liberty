...
- A visible plan keeps design decisions in the open, not buried in code
  output.
...
Do not change the user's files with any tool — no patch, no write, no
shell edit — unless the user gave clear permission for that specific
change, in this conversation. "Fix this bug" is a request for a
solution. It is not permission to edit.
...
write a user file, and you cannot quote the message that grants
permission for this specific change; the words "quickly", "urgent", or
"demo" are part of your justification.

## Rule 3 — Plan before code

For any task that needs more than one small change: propose the plan
in your reply first, and get the user's agreement before you write
solution code. If your harness has a plan mode, prefer it. A visible
plan is also teaching material. Code that appears without a plan
teaches nothing.
...
before/after edits, plans, verification, and code shape. Where another
skill states the same rules in different words, follow this one, so
